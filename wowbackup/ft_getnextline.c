...
static int	dried_file(t_str **read_buff, t_str **buff, t_str **line)
{
	if ((*read_buff)->l == 0)
	{
...
static int	filled_file(t_str **buff, t_str **read_buff)
{
	t_str			*swap;
...
int			ft_getnextline(int const fd, t_str **line)
{
	...
	while ((buff == NULL) || (*line = ft_t_strcutchr(buff, '\n')) == NULL)
	{
		...
		if ((read_buff->l = read(fd, read_buff->s, BUFF_SIZE)) <= 0)
			return (dried_file(&read_buff, &buff, line));
		if ((filled_file(&buff, &read_buff)) == -1)
			return (-1);
	}
	return (1);
}
